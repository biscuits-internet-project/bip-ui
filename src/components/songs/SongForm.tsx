...
const SongFormSchema = Yup.object().shape({
  title: Yup.string().required('Song is required'),
  author_id: Yup.string().nullable().required('Author is required'),
})
...
const SongForm: React.FC<ISongForm> = ({ id, handleClose }) => {
  ...
  useEffect(() => {
    const fetchSong = async () => {
      const data: AxiosResponse = await axios.get(
        `${process.env.REACT_APP_API_URL}/songs/${id}?edit=true`,
      )
  ...
  useEffect(() => {
    const fetchAuthors = async () => {
      ...
      const authors: IAuthorOption[] = data.data.map(
        (author: IAuthor): IAuthorOption => {
          return {
            label: author.name,
            value: author.id,
            selected: false,
  ...
  const postSong = useCallback(
    async (values: ISong, actions: FormikHelpers<ISong>) => {
      ...
      if (!id) {
        enqueueSnackbar(
          `Successfully added ${data.title} by ${data.author_name}`,
          { variant: 'success' },
        )
        handleClose('form')
      } else {
        enqueueSnackbar(
          `Successfully edited ${data.title} by ${data.author_name}`,
          { variant: 'success' },
        )
        handleClose('form')
      }
