...
const LatestShows: React.FC = () => {
  ...
  useEffect(() => {
    const fetchShows = async () => {
      const data: AxiosResponse = await axios.get(
        `${process.env.REACT_APP_API_URL}/shows?last=5`,
      )
      setShows(data.data)
    }
    fetchShows()
  ...
  return (
    <>
      <HtmlHead
        title=""
        description="Disco Biscuits setlists, reviews, ratings, band history, and more."
      />
      <div>
        <Typography variant="body1" style={{ fontSize: 16, marginBottom: 10 }}>
          Welcome to the Biscuits Internet Project 2.0 - more content than ever
          before in an easily searchable format. Go ahead...search for
          something! &nbsp; This is just the beginning – follow us on{' '}
